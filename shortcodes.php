...
function sc_meetings(){
	...
	ksort($map);
	$board_meetings = array_shift($map);
	ob_start();
	?>
							...
							<td class="documents">
								<?php
									...
									if ($agenda_id){
										...
										$url    = wp_get_attachment_url($agenda->ID);
										$mime   = mimetype_to_application(get_post_mime_type($agenda->ID));
									}else{
										$agenda = null;
									}
								?>
								<?php if ($agenda):?>
								<a class="document <?=$mime?>" href="<?=$url?>">Agenda</a>
								<?php endif;?>
								<?php
									$minutes_id = get_post_meta($meeting->ID, 'meeting_minutes', True);
									if ($minutes_id){
										$minutes = get_post($minutes_id);
										$url     = wp_get_attachment_url($minutes->ID);
										$mime    = mimetype_to_application(get_post_mime_type($minutes->ID));
									}else{
										$minutes = null;
									}
								?>
								<?php if ($minutes):?>
								<a class="document <?=$mime?>" href="<?=$url?>">Minutes</a>
								<?php endif;?>
							</td>
	...
	foreach($committees as $committee) {
		...
		<? endif ?>
			<table class="table table-striped">
				...
				<? else: ?>
					<?php meetings_prep($meetings); $row_cnt = -1; foreach($meetings as $i=>$meeting):?>
						<?php
							if (get_post_meta($meeting->ID, 'meeting_date', true)) {
								$today = getdate();
								$year  = (isset($_GET['y'])) ? $_GET['y'] : $today['year'];
								$meeting_year = date("Y", strtotime(get_post_meta($meeting->ID, 'meeting_date', true)));
								if ($meeting_year >= $year) {
						?>
							<tr class="<?=(++$row_cnt % 2) ? 'even' : 'odd';?>">
